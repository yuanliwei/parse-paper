...
public class MyWebViewSample extends Application {
	...
	private void init(Stage primaryStage) {
		...
		/**
		 * 测试能否获得javascript上面的交互内容。 可以自己写一个包含window.alert("neirong")的html进行测试。
		 * 返回的是neirong
		 */
		...
		EventHandler<ActionEvent> handler = new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				engine.load(textField.getText().startsWith("http://") ? textField.getText().trim()
						: "http://" + textField.getText().trim());
			}
		};
